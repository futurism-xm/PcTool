...
#include "StdAfx.h"
#ifdef _WIN32
#include "PcToolStorage.h" // PcTool cache leases
#endif
...
{
  #ifdef _WIN32
  if(!PcToolStorage::Root().empty()){
    const std::wstring dir=PcToolStorage::Directory(L"Cache");
    if(dir.empty()){SetLastError(ERROR_ACCESS_DENIED);return false;}
    path=(dir+L"\\").c_str();return true;
  }
